        ...
        <article class="pwt-single-package">
            ...
            <section class="pwt-section">
                ...
                <div class="pwt-meta-grid">
                    <?php if ($duration) : ?><div class="pwt-meta-chip"><strong><?php esc_html_e('Duration', 'panna-wild-tour'); ?>:</strong> <?php echo esc_html($duration); ?></div><?php endif; ?>
                    <?php if ($days) : ?><div class="pwt-meta-chip"><strong><?php esc_html_e('Days', 'panna-wild-tour'); ?>:</strong> <?php echo esc_html((string) $days); ?></div><?php endif; ?>
                    <?php if ($nights) : ?><div class="pwt-meta-chip"><strong><?php esc_html_e('Nights', 'panna-wild-tour'); ?>:</strong> <?php echo esc_html((string) $nights); ?></div><?php endif; ?>
                    <?php if ($offerPrice > 0) : ?><div class="pwt-meta-chip"><strong><?php esc_html_e('Offer Price', 'panna-wild-tour'); ?>:</strong> <?php echo esc_html('INR ' . number_format_i18n($offerPrice, 0)); ?></div><?php endif; ?>
                    <?php if ($regularPrice > 0) : ?><div class="pwt-meta-chip"><strong><?php esc_html_e('Regular Price', 'panna-wild-tour'); ?>:</strong> <?php echo esc_html('INR ' . number_format_i18n($regularPrice, 0)); ?></div><?php endif; ?>
                </div>
                <div>
                    <?php the_content(); ?>
                </div>
            ...
            <?php if ($inclusions || $exclusions) : ?>
                <section class="pwt-section pwt-grid-two">
                    <?php if ($inclusions) : ?>
                        <div>
                            <h2><?php esc_html_e('Inclusions', 'panna-wild-tour'); ?></h2>
                            <div><?php echo wp_kses_post(wpautop((string) $inclusions)); ?></div>
                        </div>
                    <?php endif; ?>
                    <?php if ($exclusions) : ?>
                        <div>
                            <h2><?php esc_html_e('Exclusions', 'panna-wild-tour'); ?></h2>
            ...
            <?php if ($itineraryRows) : ?>
                <section class="pwt-section">
                    <header class="pwt-section-header">
                        <h2><?php esc_html_e('Day-wise Itinerary', 'panna-wild-tour'); ?></h2>
                    </header>
                    <div class="pwt-itinerary-list">
                        <?php foreach ($itineraryRows as $index => $row) : ?>
                            <article class="pwt-itinerary-item">
                                <div>
                                    <p class="pwt-tag"><?php echo esc_html(sprintf(__('Day %d', 'panna-wild-tour'), $index + 1)); ?></p>
